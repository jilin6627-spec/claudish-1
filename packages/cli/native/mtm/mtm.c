...
static char statusbar_content[1024] = {0};
static time_t statusbar_mtime = 0;
...
{
    if (!statusbar_file) return;

    /* Check if file changed (stat mtime) */
    struct stat st;
    if (stat(statusbar_file, &st) == 0 && st.st_mtime != statusbar_mtime) {
        statusbar_mtime = st.st_mtime;
        FILE *f = fopen(statusbar_file, "r");
        if (f) {
            char line[1024] = {0};
            char last[1024] = {0};
            ...
            if (len > 0 && last[len-1] == '\n') last[len-1] = '\0';
            memcpy(statusbar_content, last, sizeof(statusbar_content));

            /* First time content appears: shrink pane by 1 row to make room */
            if (!statusbar_visible && statusbar_content[0] != '\0') {
                statusbar_visible = true;
                reshape(root, 0, 0, LINES - 1, COLS);
            }
        }
    }
     ...
     * Status format: segments separated by \t (tab).
     * Each segment: "color_code:text" where color_code is:
     *   M=magenta, C=cyan, G=green, R=red, D=dim, W=white */
    int y = LINES - 1;
    int x = 0;
    move(y, 0);
    clrtoeol();

    char *seg = statusbar_content;
    while (*seg) {
        /* Find color prefix (single char + colon) */
        short fg = COLOR_WHITE, bg = -1;
        int bold = 0;
        if (seg[1] == ':') {
            switch (seg[0]) {
                case 'M': fg = COLOR_MAGENTA; bg = COLOR_MAGENTA; bold = 1; break;
                case 'C': fg = COLOR_CYAN;    bg = COLOR_CYAN;    bold = 1; break;
                case 'G': fg = COLOR_GREEN;   bg = COLOR_GREEN;   bold = 1; break;
                case 'R': fg = COLOR_RED;     bg = COLOR_RED;     bold = 1; break;
                case 'D': fg = 8;             bg = 237;           bold = 0; break;
                case 'W': fg = COLOR_WHITE;   bg = -1;            bold = 0; break;
            }
            seg += 2;
        }

        /* Find end of segment (tab or end of string) */
        char *end = strchr(seg, '\t');
        size_t len = end ? (size_t)(end - seg) : strlen(seg);

        /* Draw pill: colored bg with text */
        short pair = mtm_alloc_pair(bg >= 0 ? COLOR_WHITE : fg, bg >= 0 ? bg : -1);
        attron(COLOR_PAIR(pair));
        if (bold) attron(A_BOLD);
        mvaddnstr(y, x, seg, len);
    ...
    /* Draw expanded log view if active */
    if (statusbar_expanded) {
        const char *logfile = diag_log_file ? diag_log_file : statusbar_file;
        if (!logfile) return;

        int panel_h = MIN(EXPANDED_ROWS, LINES - 2); /* leave room for pane + bar */
        int panel_y = LINES - 1 - panel_h; /* above the status bar */
        ...
        while (fgets(linebuf, sizeof(linebuf), f)) {
            ...
            if (l > 0 && linebuf[l-1] == '\n') linebuf[l-1] = '\0';
            memcpy(lines[total % panel_h], linebuf, sizeof(linebuf));
            total++;
        }
        fclose(f);
        ...
        /* Draw top border */
        short borderpair = mtm_alloc_pair(COLOR_MAGENTA, 235);
        attron(COLOR_PAIR(borderpair));
        move(panel_y, 0);
        for (int col = 0; col < COLS; col++) addch(ACS_HLINE);
        const char *title = " claudish diagnostics ";
        mvaddstr(panel_y, 2, title);
...
static void
toggle_statusbar_expand(void)
{
    if (!statusbar_visible) return;
    statusbar_expanded = !statusbar_expanded;

    int bar_rows = statusbar_expanded ? (1 + MIN(EXPANDED_ROWS, LINES - 2)) : 1;
    reshape(root, 0, 0, LINES - bar_rows, COLS);
    draw_statusbar();
}
