...
### Fixed
- 🐛 **Kimi turn-2 crash** (`tool_reference` not supported) — `AnthropicPassthroughAdapter.convertMessages()` now strips Claude-internal `tool_reference` content blocks before forwarding to Kimi and other Anthropic-compatible providers. These blocks are generated by Claude Code's deferred tool loading (ToolSearch) and are not part of the public Anthropic API spec. Any `tool_result` whose content is entirely `tool_reference` items gets a minimal `""` text placeholder so the conversation remains valid.
- 🐛 **`qwen3.5-plus-02-15` invalid model ID on OpenRouter** — `getOpenRouterHandler()` was calling `parseModelSpec()` on already-resolved `vendor/model` IDs (e.g. `qwen/qwen3.5-plus-02-15`), which matched the `^qwen` native provider pattern and stripped the `qwen/` vendor prefix before sending to OpenRouter. Now the vendor prefix is only stripped for explicit `@`-syntax targets (e.g. `openrouter@google/gemini-2.0-flash`); pre-resolved `vendor/model` strings are passed through unchanged.
